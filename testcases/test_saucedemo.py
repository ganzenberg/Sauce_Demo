...
class Test_Saucedemo(Webelements):
    ...
    def test_order_product(self, setup):
        ...
        self.lp = Sauce_Demo(self.driver)
        self.lp.login_saucedemo(url=self.baseURL, username=self.Uname1, password=self.Pword)
        self.lp.order_product(fname=self.fname, lname=self.lname, zip_code=self.zip_code, act_value=self.act_value)
        ...
        if complete_msg == self.check_out_msg:
            ...
            self.driver.save_screenshot("/Users/ganeshkumar/PycharmProjects/Sauce_Demo/screenshots/test_order_product.png")
            assert True
        else:
